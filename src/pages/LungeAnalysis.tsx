/**
 * Dumbbell Reverse Lunge Analysis Page
 * User can watch sample video, record their lunge, and get detailed feedback
 */
...
import {
  ...
  ZoomIn, ZoomOut, Loader2, Volume2, VolumeX, Eye, EyeOff
} from 'lucide-react';
...
/**
 * Upload video for lunge analysis
 */
const uploadVideoForLungeAnalysis = async (
  videoBlob: Blob,
  filename: string = 'lunge_video.webm',
  ...
  formData.append('analyze_squat', 'false');
  formData.append('analyze_pushup', 'false');
  formData.append('analyze_barbell', 'false');
  formData.append('analyze_lunge', 'true');
...
export default function LungeAnalysis() {
  ...
  const [cameraAngle, setCameraAngle] = useState<'front_view' | 'side_view'>('side_view'); // Default to side view
  ...
  // Voice feedback
  const { playFeedback, stop, isPlaying } = useVoiceFeedback('lunge', cameraAngle);
  ...
  const handleUploadVideo = async () => {
    ...
    try {
      ...
      const result = await uploadVideoForLungeAnalysis(recordedBlob, 'lunge_recording.webm', cameraAngle, newSessionId);
  ...
  // Helper to detect positive feedback text - CHECK NEGATIVE FIRST
  const isPositiveFeedback = (feedback?: string): boolean => {
    if (!feedback) return false;
    const lower = feedback.toLowerCase();

    // Check negative keywords FIRST - if any found, return false
    const negativeKeywords = ['chưa hạ', 'chưa đủ', 'nghiêng người', 'quá sâu', 'quá nông', 'bị cong', 'cong quá', 'hạ sâu hơn'];
    if (negativeKeywords.some(k => lower.includes(k))) {
      return false;
    }

    // Then check for positive keywords
    const positiveKeywords = ['rất tốt', 'rất chuẩn', 'lý tưởng', 'thẳng và ổn định', 'ổn định'];
    return positiveKeywords.some(k => lower.includes(k));
  };
      ...
      <div className="relative max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-white mb-2 drop-shadow-lg">
            Phân Tích Động Tác Dumbbell Reverse Lunge
          </h1>
            ...
            <Card className="transition-all duration-300">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Video Mẫu Dumbbell Reverse Lunge
                </CardTitle>
                <CardDescription>
              ...
              <CardContent>
                <video
                  ...
                  src={`${API_BASE_URL}/sample-videos/lunge${cameraAngle === 'front_view' ? '' : '-side'}`}
                >
                  Trình duyệt không hỗ trợ video
                </video>
                <div className="mt-4 space-y-2 text-sm text-white/80">
                  <p>✓ Lưng thẳng, không cúi gập</p>
                  <p>✓ Chân sau lùi xa, gối gần chạm đất</p>
                  <p>✓ Góc đầu gối 80°-115° khi xuống thấp</p>
                  <p>✓ Đứng thẳng hoàn toàn khi trở về</p>
                </div>
              </CardContent>
            </Card>
          )}
            ...
            <Card className={`${showSampleVideo ? '' : 'shadow-xl'}`}>
              <CardHeader>
                ...
                <CardDescription>
                  ...
                  {recordingState === 'recording' && '🔴 Đang quay... Thực hiện động tác lunge'}
                  ...
                  <div className="mt-4">
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="front_view" id="lunge-front-view" />
                        <Label htmlFor="lunge-front-view" className="cursor-pointer">
                          Góc Chính Diện
                        </Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="side_view" id="lunge-side-view" />
                        <Label htmlFor="lunge-side-view" className="cursor-pointer">
                          Góc Ngang
                        </Label>
                      </div>
                    </RadioGroup>
        ...
        {processingResult && (
          <div className="space-y-6">
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <Button
                    onClick={() => {
                      ...
                      link.download = 'lunge_analysis.webm';
                      link.click();
                    }}
                    variant="outline"
                    size="sm"
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-4xl font-bold text-purple-600">
                    {processingResult.total_lunges || 0}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-4xl font-bold text-green-600">
                    {processingResult.valid_lunges || 0}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-4xl font-bold text-red-600">
                    {processingResult.invalid_lunges || 0}
                  </p>
                </CardContent>
              </Card>
            </div>
            ...
            {processingResult.lunge_rep_feedbacks && processingResult.lunge_rep_feedbacks.length > 0 && (
              <Card>
                <CardHeader>
                    ...
                    <Button
                      onClick={() => {
                        if (isPlaying) {
                          ...
                          playFeedback(processingResult.lunge_rep_feedbacks || []);
                        }
                      }}
                      variant="outline"
                      size="sm"
                ...
                <CardContent>
                  <div className="overflow-x-auto">
                    <table className="w-full border-collapse">
                      ...
                      <tbody>
                        {processingResult.lunge_rep_feedbacks.map((rep: any) => (
