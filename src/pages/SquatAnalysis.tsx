/**
 * Squat Analysis Page
 * User can watch sample video, record their squat, and get detailed feedback
 */
...
import { VideoRecorder, uploadVideoForAnalysis, RepFeedback, VideoProcessingResponse } from '@/services/videoService';
...
export default function SquatAnalysis() {
  ...
  const loadSavedState = () => {
    const saved = localStorage.getItem('squatAnalysisState');
    if (saved) {
      try {
  ...
  // Voice feedback
  const { playFeedback, stop, isPlaying } = useVoiceFeedback('squat', cameraAngle);
  ...
  useEffect(() => {
    if (processingResult) {
      localStorage.setItem('squatAnalysisState', JSON.stringify({
        recordingState,
        processingResult
      }));
    }
  ...
  const handleUploadVideo = async () => {
    ...
    try {
      ...
      const result = await uploadVideoForAnalysis(recordedBlob, 'squat_recording.webm', cameraAngle, newSessionId);
  ...
  const handleReset = () => {
    ...
    setError('');
    // Clear localStorage
    localStorage.removeItem('squatAnalysisState');
    if (videoPreviewRef.current) {
      ...
      <div className="relative max-w-7xl mx-auto p-6">
            ...
            <Card className="transition-all duration-300">
              ...
              <CardContent>
                <video
                  ...
                  src={`${API_BASE_URL}/sample-videos/squat${cameraAngle === 'side_view' ? '-side' : ''}`}
                >
                  Trình duyệt không hỗ trợ video
                </video>
                <div className="mt-4 space-y-2 text-sm text-white/80">
                  <p>✓ Lưng thẳng, ngực mở</p>
                  <p>✓ Gối không vượt quá mũi chân</p>
                  <p>✓ Hông xuống thấp đến góc 90°</p>
                  <p>✓ Đứng thẳng hoàn toàn khi lên</p>
                </div>
              </CardContent>
            </Card>
          )}
            ...
            <Card className={`${showSampleVideo ? '' : 'shadow-xl'}`}>
              <CardHeader>
                ...
                <CardDescription>
                  ...
                  {recordingState === 'recording' && '🔴 Đang quay... Thực hiện động tác squat'}
                  ...
                  <div className="mt-4">
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="front_view" id="front-view" />
                        <Label htmlFor="front-view" className="cursor-pointer">
                          Góc Chính Diện
                        </Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="side_view" id="side-view" />
        ...
        {processingResult && recordingState === 'completed' && (
          <div className="space-y-6">
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <Button
                    onClick={() => {
                      ...
                      link.download = 'squat_analysis.mp4';
                      link.click();
                    }}
                    variant="outline"
                    size="sm"
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-4xl font-bold text-blue-600">
                    {processingResult.total_squats || 0}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-4xl font-bold text-green-600">
                    {processingResult.valid_squats || 0}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-4xl font-bold text-red-600">
                    {processingResult.invalid_squats || 0}
                  </p>
                </CardContent>
              </Card>
            </div>
            ...
            {processingResult.rep_feedbacks && processingResult.rep_feedbacks.length > 0 && (
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        Xem feedback cho từng lần thực hiện
                      </CardDescription>
                    </div>
                    <Button
                      onClick={() => {
                        if (isPlaying) {
                          ...
                          playFeedback(processingResult.rep_feedbacks || []);
                        }
                      }}
                      variant="outline"
                      size="sm"
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b">
                          ...
                          <th className="text-left p-3 font-semibold">Độ Sâu</th>
                          {/* Chỉ hiển thị cột Tư Thế Lưng khi ở side view */}
                          {cameraAngle === 'side_view' && (
                            <th className="text-left p-3 font-semibold">Tư Thế Lưng</th>
                          )}
                          <th className="text-left p-3 font-semibold">Góc Gối Thấp Nhất</th>
                        </tr>
                      </thead>
                      <tbody>
                        {processingResult.rep_feedbacks.map((rep: RepFeedback) => (
                          <tr key={rep.rep_id} className="border-b hover:bg-gray-50">
                            ...
                            <td className={`p-3 ${getStatusColor(isPositiveFeedback(rep.depth_feedback))}`}>
                              <div className="flex items-center gap-2">
                                {getStatusIcon(isPositiveFeedback(rep.depth_feedback))}
                                <span className="text-sm">{rep.depth_feedback}</span>
                              </div>
                            </td>
                            {/* Chỉ hiển thị cột Tư Thế Lưng khi ở side view */}
                            {cameraAngle === 'side_view' && (
                              <td className={`p-3 ${getStatusColor(isPositiveFeedback(rep.back_feedback))}`}>
                                <div className="flex items-center gap-2">
                                  {getStatusIcon(isPositiveFeedback(rep.back_feedback))}
                                  <span className="text-sm">{rep.back_feedback}</span>
                                </div>
                              </td>
                            )}
                            <td className="p-3 text-sm">
                              {rep.lowest_knee_angle.toFixed(1)}°
                            </td>
                          </tr>
                        ))}
                      </tbody>
